...
- `Faction = Player`로 고정.
- **입력**: `PlayerInputHandler`(WASD 이동 + 좌클릭 공격)가 `PlayerController.OnMoveInput`/`OnAttackInput`을 호출한다. 프로젝트의 다른 입력 핸들러와 같은 관례(`Keyboard.current`/`Mouse.current` 직접 폴링, `WindowManager.IsAnyWindowOpen`이면 이동 정지)를 따르고, 스톡 `InputSystem_Actions.inputactions`는 쓰지 않는다. Sprint/Jump는 `PlayerLocomotion`이 스스로 처리한다(Space/Shift를 직접 읽으므로 씬마다 연결하는 `windowManager`로 같은 게이팅을 건다 — 대화·창이 열려 있으면 점프/달리기 불가, 비워 두면 게이팅 없음). 현재 이동은 카메라 기준이 아닌 월드 좌표(X/Z) 기준이다 — 카메라/시점 시스템이 아직 없음.
